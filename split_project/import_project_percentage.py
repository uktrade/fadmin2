...
class UploadProjectPercentages:
    ...
    def display_row_count(self, row_number):
        if not row_number % 100:
            ...
            set_file_upload_feedback(
                self.file_upload,
                f"Processing row {row_number} " f"of {self.rows_to_process}.",
            )

    def set_final_status(self, check_financial_code):
        final_status = FileUpload.PROCESSED
        if check_financial_code.warning_found:
            final_status = FileUpload.PROCESSEDWITHWARNING
    ...
    def apply_percentages(self):
        for month_obj in self.month_dict.values():
            if month_obj.actual_loaded:
                handle_split_project(
                    month_obj.financial_period_code, ForecastMonthlyFigure
                )
    ...
    def read_percentages(self):
        ...
        for percentage_row in self.worksheet.rows:
            ...
            if row_number == 1:
                ...
                # Ignore first row, the headers have been processed already
                continue
            self.display_row_count(row_number)
            ...
            if not self.check_financial_code.error_found:
                try:
                    ...
                    self.check_financial_code.record_error(row_number, str(ex))

        return (
            self.set_final_status(self.check_financial_code)
            != FileUpload.PROCESSEDWITHERROR
        )
