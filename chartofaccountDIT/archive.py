from core.archive import archive_generic

from .models import (
    Analysis1,
    Analysis2,
    CommercialCategory,
    ExpenditureCategory,
    FCOMapping,
    HistoricAnalysis1,
    HistoricAnalysis2,
    HistoricCommercialCategory,
    HistoricExpenditureCategory,
    HistoricFCOMapping,
    HistoricInterEntity,
    HistoricNaturalCode,
    HistoricProgrammeCode,
    HistoricProjectCode,
    InterEntity,
    NaturalCode,
    ProgrammeCode,
    ProjectCode,
)


def archive_project_code(year):
    return archive_generic(year, HistoricProjectCode, ProjectCode)


def archive_programme_code(year):
    return archive_generic(year, HistoricProgrammeCode, ProgrammeCode)


def archive_expenditure_category(year):
    return archive_generic(year, HistoricExpenditureCategory, ExpenditureCategory)


def archive_inter_entity(year):
    return archive_generic(year, HistoricInterEntity, InterEntity)


def archive_fco_mapping(year):
    return archive_generic(year, HistoricFCOMapping, FCOMapping)


def archive_commercial_category(year):
    return archive_generic(year, HistoricCommercialCategory, CommercialCategory)


def archive_analysis_1(year):
    return archive_generic(year, HistoricAnalysis1, Analysis1)


def archive_analysis_2(year):
    return archive_generic(year, HistoricAnalysis2, Analysis2)


def archive_natural_code(year):
    return archive_generic(year, HistoricNaturalCode, NaturalCode)


def archive_all(year):
    archive_project_code(year)
    archive_programme_code(year)
    archive_expenditure_category(year)
    archive_inter_entity(year)
    archive_fco_mapping(year)
    archive_commercial_category(year)
    archive_analysis_1(year)
    archive_analysis_2(year)
    archive_natural_code(year)
