...
class Analysis2CodeTests(TestCase):
    @override_settings(
    ...
    def test_data_returned_in_response(self):
        ...
        archived_analysis2_code = HistoricalAnalysis2Factory.create(
        ...
        cols = rows[1].split(",")
        assert str(cols[0]) == str(analysis2_code)
        # Check the archived value
        cols = rows[2].split(",")
        assert str(cols[0]) == str(archived_analysis2_code)
